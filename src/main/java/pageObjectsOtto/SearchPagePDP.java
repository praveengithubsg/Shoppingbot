...
public class SearchPagePDP extends CucumberRunnerCilgro {
	/**
	...
	private Logger log = Logger.getLogger(SearchPagePDP.class.getName());

	/**
	 * Constructor to initialize page objects
	 **/
	public SearchPagePDP() {
	...
	@FindBy(xpath = "//button[@id='btnQuickOrderBottom']")
	...
	@FindBy(xpath = "//button[@type='submit' and @class='btn btn-big btn-action btn-add-to-basket ellipsis']")
	...
	public void clickIncreaseQtyButton() {
		...
		try {
			...
			for (int row = 1; row < dataReader.rowCount(config.getExcelSheetName()); row++) {
				...
				boolean searchFlag=homePage.clickSearchSuggestion();
				String prijs= null;
				int intAANTAL = 0;
				...
				if (commonMethods.isElementPresent(txtPdpItemQty)) {

					txtPdpItemQty.clear();
					log.info("Cleared default quantity from item quantity text box");
					...
					if (intMOQ != 1) {
						...
						commonMethods.clearAndSendKeys(txtPdpItemQty, AANTAL);
